...
```json
{
  ...
  "function": {
    "name": "reason",
    "description": "Structured thinking, data processing, or hypothesis verification. No side effects — the output feeds your own reasoning and is not shown to the user.\n\nCalls run in submission order, so a call that depends on an earlier one can still be issued in the same response. Split into a separate response only when you must see a result before deciding what to do next.",
    "parameters": {
      "type": "object",
      "properties": {
        "runtime": {
