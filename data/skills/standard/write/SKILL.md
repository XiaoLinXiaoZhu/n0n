---
description: write 工具的使用原则和文件修改决策树。
activation: init
order: 120
---
...
文件组织良好（单一职责、短小）时，任何修改几乎都涉及文件 50%+ 的内容——此时"修改"和"重写"没有本质区别。`write` 的唯一成本是 token 量，但短小的文件让这个成本可以忽略。若觉得重写"太浪费 token"，那是拆分信号——拆分原则见 file-organization skill。
...
4. **遗留代码、不值得重构** → unified diff + `git apply` 作为降级方案（见下文）
