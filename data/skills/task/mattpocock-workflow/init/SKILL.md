---
description: 初始化仓库配置：建立 issue tracker、领域词汇表、架构决策记录等基础设施。当用户说"初始化"、"配置仓库"、"设置项目"或由 @mwf 自动路由到 init 阶段时使用。
activation: manual
alias: mwf-init
---
...
> issue tracker 是本仓库 issue（任务单元）的存放位置。`dispatch` 等用于输出 issue 的 skill 需要知道是调用 GitHub CLI 创建 issue、写入本地 `.scratch/` 文件，还是其他工作流。

选项（基于 `git remote` 推断默认值）：
- **GitHub** — issue 作为 GitHub Issues 管理，使用 `gh` CLI
- **本地 markdown** — issue 作为 `.scratch/<feature>/` 下的 markdown 文件，适合独立项目或无远程仓库

#### B — 领域词汇表

> 领域词汇表（`CONTEXT.md`）定义项目使用的精确术语。`plan` 和 `resolve-hitl` 在质询过程中会维护它。告诉 agent 去哪里找。
...
> ADR 记录难以逆转的架构取舍。默认位置是 `docs/adr/`。确认用户是否接受，或指定其他路径。
...
<summary>。详见 `docs/agents/issue-tracker.md`。
...
**docs/agents/issue-tracker.md** — issue tracker 配置详情（根据选择写入对应模板）。
...
```
【阶段】Init — 完成
【Issue tracker】GitHub / 本地 markdown
【领域词汇表】单上下文 / 多上下文
【ADR 位置】docs/adr/
