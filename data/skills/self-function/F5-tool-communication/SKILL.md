...
| F5.3 输出预处理 | 倾倒大段原始输出；混淆项目发现与大文件阅读；为获取输出重跑有副作用命令 | 6 | 5 | 按意图选择搜索、提取、提高预算或读取 artifact | 自检输出预算与读取方式是否匹配目标 |
...
- 独立调用可在同一响应中批量发出以减少模型轮次；执行器可能按冲突策略顺序执行
- write 可与 observe/reason/act 同批发出

其他工具使用规则：
- 优先用 `rg`（ripgrep）而非 `grep`——更快、默认递归、自动尊重 .gitignore
- 在未知的大型项目中发现候选位置时使用 `rg`、`fd` 等搜索工具；“目标未知”不等于必须顺序读取文件
- 已知要找什么或数据结构明确时优先 `rg`、`jq`、数据库查询或脚本提取
- 确实需要完整且规模可控的命令结果时，显式设置 `output_tokens`；其预算覆盖 stdout 与 stderr 合计
...
1. **项目发现**：使用 `rg`、`fd` 等跨文件搜索候选位置；大量匹配本身有意义时可提高 `output_tokens`
2. **结构化提取**：使用 `rg`、`jq`、数据库查询或脚本直接提取
3. **需要完整结果**：预估规模可控时提高 `output_tokens`，避免无意义的多轮分段
4. **已有 artifact**：昂贵、非幂等或有副作用的命令不得为扩大输出而重跑；读取并过滤其 stdout/stderr
5. **明确范围**：使用系统标准范围命令读取；多个互不依赖的范围可在同一响应中批量调用

禁止：
- 在没有预算判断时对未知大小文件或 artifact 使用 `cat`、`type` 等整文件输出
- 仅为获得更多输出而重跑可能改变状态的 `act`
- 把“项目内容未知”误判为必须从某个文件开头顺序读取
...
- **F5.3**: 项目发现是否使用搜索？完整结果是否显式选择了合适预算？有副作用命令截断后是否读取 artifact 而非重跑？
