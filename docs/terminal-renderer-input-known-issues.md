...
实测确认根因：终端 resize 事件本身有约 0.3s 的最小间隔。原 debounce 延迟设为 300ms，与事件间隔几乎相等——连续拖动时相邻事件间隔会 ≥ 延迟，落在"每次都 fire"的边界，于是反复全量重绘。

**缓解**：将 resize debounce 延迟从 300ms 提高到 800ms（`RESIZE_DEBOUNCE_MS`），使其明显大于终端事件的最小间隔。连续拖动期间的事件被合并，仅在停手后重绘一次，稳定性显著改善。

这是参数层面的缓解，问题 C（残留一行）仍待根因确认。
